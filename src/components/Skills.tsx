...
export const Skills: React.FC = () => {
  const skills = [
    { name: 'DevOps', icon: Zap, level: 'Advanced', color: 'text-yellow-500' },
    { name: 'Cloud Computing', icon: Cloud, level: 'Advanced', color: 'text-blue-500' },
    { name: 'Docker', icon: Container, level: 'Advanced', color: 'text-blue-600' },
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16">
          {skills.map((skill, index) => (
            <div
              key={skill.name}
              className="group relative p-6 royal-luxury-card rounded-xl transition-all duration-500 transform hover:scale-105 hover:shadow-2xl cursor-pointer royal-tech-glow"
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              <div className="text-center">
                <skill.icon className={`w-8 h-8 mx-auto mb-3 ${skill.color.includes('yellow') ? skill.color : skill.color.replace('text-', 'text-royal-')} group-hover:scale-110 transition-transform duration-300`} />
              ...
              {/* Skill level bar */}
              <div className="absolute bottom-0 left-0 right-0 h-1 bg-yellow-200 dark:bg-gray-700 rounded-b-xl overflow-hidden">
                <div 
                  className={`h-full ${skill.color.includes('yellow') ? 'bg-yellow-500' : 'bg-blue-900'} transition-all duration-500 group-hover:w-full`}
                  style={{ 
        ...
        <div className="text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            {tools.map((tool, index) => (
              <span
                key={tool}
                className="px-4 py-2 bg-yellow-200 dark:bg-blue-900/30 text-royal-navy dark:text-yellow-300 rounded-full text-sm font-medium hover:bg-yellow-300 dark:hover:bg-blue-800/50 transition-all duration-300 transform hover:scale-105"
